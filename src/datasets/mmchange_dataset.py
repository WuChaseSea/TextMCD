...
class MMChangeData(BaseData):
    ...
    def prepare(data_dir, list_file=None, img_suffix=None, label_suffix=None, num_classes=2, **dataset_cfg):
        ...
        text_a_json = os.path.join(train_data_path, "tinyllava_a.json")
        text_b_json = os.path.join(train_data_path, "tinyllava_b.json")
        with open(text_a_json, "r", encoding="utf-8") as f:
            df_text_a = json.load(f)
        with open(text_b_json, "r", encoding="utf-8") as f:
            df_text_b = json.load(f)
        
        # model, preprocess = clip.load("./pretrained_models/clip/ViT-L-14.pt")
        text_token_a = [clip.tokenize(df_text_a[Path(name).name], context_length=77, truncate=True) for name in train_images_A]
        text_token_b = [clip.tokenize(df_text_b[Path(name).name], context_length=77, truncate=True) for name in train_images_A]
        
        try:
            df = pd.DataFrame({"image_file_A": train_images_A, "image_file_B": train_images_B, "mask_file": train_labels, "text_token_a": text_token_a, "text_token_b": text_token_b})
        except:
    ...
    def __getitem__(self, idx):
        ...
        return img, torch.Tensor(mask).long(), text_token_a, text_token_b, img_ori, os.path.basename(mask_file)
