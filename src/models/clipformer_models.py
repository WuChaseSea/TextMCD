...
from src.models.mmseg.utils.se_layer import SELayer_v2 as SELayer
from src.models.mmseg.utils.clip_func import clip_infer, init_clip

from src.models.mmseg.utils.untils import tokenize

from src.models.mmseg.rmodels import MODELS
...
class CLIPFormer(BaseSegmentor):
    ...
    def __init__(self,
                 ...
                 minus_channel = [96, 192, 384, 770, 1024],
        ...
        self.name = "CLIPFormer"
        backbone = OmegaConf.to_container(backbone)
        text_encoder = OmegaConf.to_container(text_encoder)
        context_decoder = OmegaConf.to_container(context_decoder)
        decode_head = OmegaConf.to_container(decode_head)
        neck = OmegaConf.to_container(neck)
        ...
        self.minus_conv = nn.Sequential(ConvModule(
                    ...
                    kernel_size=1),
                    ConvModule(
                    in_channels=self.minus_channel[4],
                    out_channels=256,
                    kernel_size=1)
                    )
        self.channel_att = nn.Sequential(SELayer(768, 256), SELayer(768, 256), SELayer(768, 256), SELayer(768, 256), SELayer(768, 256))
    ...
    def extract_feat(self, inputs: Tensor, alpha=None) -> List[Tensor]:
        """Extract features from images."""
        if alpha is not None:
            x = self.backbone(inputs.float(), alpha.float())
        else:
            x = self.backbone(inputs.float())
        return x
    ...
    def get_cls_text(self, jsonA, jsonB, train=True):
        textA = []
        textB = []

        # jsonA_new, jsonB_new = [], []
        # for i in range(len(jsonA[0])):
        #     jsonA_tmp = []
        #     for j in range(len(jsonA)):
        #         jsonA_tmp.append(jsonA[j][i])
        #     jsonA_new.append(jsonA_tmp)
        # #
        # for i in range(len(jsonB[0])):
        #     jsonB_tmp = []
        #     for j in range(len(jsonB)):
        #         jsonB_tmp.append(jsonB[j][i])
        #     jsonB_new.append(jsonB_tmp)
        # jsonA = jsonA_new
        # jsonB = jsonB_new
        # import pdb;pdb.set_trace()
        for i in range(len(jsonA)):
            if jsonA[i] == 'nothing':
                foreA = '.'.join(['this image nothing changed'])
            else:
                # import pdb;pdb.set_trace()
                foreA = ', '.join(['this image contains '] + [jsonA[i]])
            # if train:
            #     foreA = ', '.join([jsonA[i]])
            # else:
            #     foreA = ', '.join(['remote sensing image foreground objects']+[jsonA[i]])
            backA = ', '.join(['it is a remote sensing image'])
            textA.append(torch.cat([tokenize(c, context_length=64) for c in [backA, foreA]]).unsqueeze(0))
        for i in range(len(jsonB)):
            if jsonB[i] == 'nothing':
                foreB = '.'.join(['this image nothing changed'])
            else:
                foreB = ', '.join(['this image contains,'] + [jsonB[i]])
            # if train:
            #     foreB = ', '.join(['remote sensing image foreground objects']+[jsonB[i]])
            # else:
            #     foreB = ', '.join(['remote sensing image foreground objects']+[jsonB[i]])
            backB = ', '.join(['it is a remote sensing image'])
    ...
    def _forward(self,
        ...
        inputs, jsonA, jsonB = inputs
        x_shape = inputs.shape
        alpha = None
        if x_shape[1] == 7:
            alpha = inputs[:, -1, :, :][:, None , ...]
        inputsA = inputs[:, :3, :, :]
        # inputsA = torch.cat((inputsA, inputs[:, -1, :, :].unsqueeze(1)), dim=1)
        inputsB = inputs[:, 3:6, :, :]
        # inputsB = torch.cat((inputsB, inputs[:, -1, :, :].unsqueeze(1)), dim=1)
        if alpha is not None:
            xA = self.extract_feat(inputsA, alpha)
            xB = self.extract_feat(inputsB, alpha)
        else:
            xA = self.extract_feat(inputsA)
            xB = self.extract_feat(inputsB)
        ...
        textA, textB = self.get_cls_text(jsonA, jsonB)
        text_embeddingsA, x_clipA, score_mapA = self.after_extract_feat_clip(xA, textA)
        text_embeddingsB, x_clipB, score_mapB = self.after_extract_feat_clip(xB, textB)

        x_orig = [torch.cat([x_clipA[i], x_clipB[i]], dim=1) for i in range(len(x_clipA))]
        # import ipdb;ipdb.set_trace()
        # self.minus_conv[3](torch.abs(x_clipA[3] - x_clipB[3]))
        try:
            x_minus = [self.minus_conv[i](torch.abs(x_clipA[i]-x_clipB[i])) for i in range(len(x_clipA))]
        ...
        #                                       self.test_cfg)
        _, _, tmp_h, tmp_w = x[0].shape
        x[-1] = F.interpolate(x[-1], size=(tmp_h*2, tmp_w*2))
        seg_logits = self.decode_head(x, text_embeddingsA, text_embeddingsB)
        seg_logits = resize(input=seg_logits, size=x_shape[2:], mode='bilinear', align_corners=False)
        torch.cuda.synchronize()
        # print(f"seg logits: max {seg_logits.max()}, min {seg_logits.min()}")
        return seg_logits
