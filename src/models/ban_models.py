...
from src.models.mmseg.utils.ops import resize


class BANModel(BaseSegmentor):
    ...
    def __init__(self,
        ...
        image_encoder = OmegaConf.to_container(image_encoder)
        decode_head = OmegaConf.to_container(decode_head)

        if pretrained is not None:
            image_encoder['init_cfg'] = {
                'type': 'Pretrained_Part',
                'checkpoint': pretrained
            }
            # decode_head['init_cfg'] = {
            #     'type': 'Pretrained_Part',
            #     'checkpoint': pretrained
            # }
    ...
    def _forward(self,
        ...
        fm_img_from, fm_img_to = img_from, img_to
        if isinstance(self.encoder_resolution, omegaconf.dictconfig.DictConfig):
            self.encoder_resolution = OmegaConf.to_container(self.encoder_resolution)
        if self.asymetric_input:
            fm_img_from = F.interpolate(fm_img_from, **self.encoder_resolution)
            fm_img_to = F.interpolate(fm_img_to, **self.encoder_resolution)
        fm_feat_from = self.extract_feat(fm_img_from)
        fm_feat_to = self.extract_feat(fm_img_to)

        output = self.decode_head.forward([img_from, img_to, fm_feat_from, fm_feat_to])
        output = resize(input=output, size=inputs.shape[2:], mode='bilinear', align_corners=False)
        return output
